...
import { approveChangeRequest, rejectChangeRequest } from "@/modules/admin/actions";
                    ...
                    <table className="w-full min-w-[420px] text-sm">
                      ...
                      <tbody>
                        {Object.entries(changes).map(([field, change]) => (
                          <tr key={field} className="border-b last:border-0 align-top">
                            ...
                            <td className="px-3 py-2">{renderValue(change?.to)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {closed.length > 0 && (
        <details className="mt-8">
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {closed.map((request) => (
              <li key={request.id} className="rounded-lg border bg-card px-4 py-2.5">
                <span className="font-medium">{request.clinics?.name ?? "Clinic"}</span>{" "}
